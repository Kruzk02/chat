...
/**
 * Utility class for serializing and deserializing simple messages with a header and a UTF-8 encoded
 * payload.
 *
 ...
 * <ul>
 *   <li>1 byte header
 *   <li>4-byte integer representing payload length
 *   <li>Payload as UTF-8 encoded bytes
 * </ul>
 */
public class MessageParser {
  ...
  /**
   * Writes a message to the given {@link DataOutputStream}. The message is composed of a 1-byte
   * header, a 4-byte length of the payload, and the UTF-8 encoded payload bytes.
   *
   ...
   */
  public static void writeMessage(DataOutputStream out, byte header, String payload)
      throws IOException {
  ...
  /**
   * Reads a message from the given {@link DataInputStream}. Expects the message format to be:
   *
   * <ul>
   *   <li>1 byte header
   *   <li>4 bytes indicating the length of the payload
   *   <li>UTF-8 encoded payload bytes
   * </ul>
   *
